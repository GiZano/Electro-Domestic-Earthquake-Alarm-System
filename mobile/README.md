...
- **AI Emergency Reports:** When an alert is confirmed, the backend generates a report via a local Ollama LLM and pushes it over the `ai_reports` WebSocket channel. The app displays an inline banner for the latest report and a card in the alert history feed (summary + recommendations), with an "AI Report Unavailable" badge if generation failed. Reports can be re-fetched via `GET /reports/{alert_id}` after a reconnect.
